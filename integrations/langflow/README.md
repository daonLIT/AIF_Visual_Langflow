...
- 서버: GPU 서버(ARM64, Docker 28.3·Compose v2)에 `deploy/` 구성 그대로 올렸다. 주소는 정식 도메인이 없어 임시로 `https://210-115-229-70.sslip.io`.
- Let's Encrypt 인증서 발급(HTTP-01, 바깥에서 80 도달 확인), http→https 308, 익명 401.
- `smoke_check.py` 14/14 통과.
- 개발 모드 셸을 이 서버에 연결: 사건 목록·열기, 전체 수락 후 저장 → 웹에서 새로고침하면 같은 결과. Flow 실행 → 게시 "저장됨" → [이 결과 보기] 열림.
- 서버의 코드는 저장소가 비공개라 `git archive` 사본으로 올렸다(`~/aif-central`, git 저장소 아님). 공개 뒤에는 clone 으로 바꾼다.
- 휴대폰 모바일 데이터(학교 밖)에서 접속 확인.
- 아직: 설치본(`npm run dist`)으로 연결.

### 확인하지 않은 것 / 운영 전에 필요한 것

- 정식 도메인. 지금은 sslip.io 임시 주소다(위 "운영 배포").
- 코드 서명 인증서와 서명된 설치 파일(SmartScreen 경고 없애기). 자동 업데이트(서명·배포 서버 필요)는 넣지 않았다.
- 설치본을 HTTPS 공개 서버에 연결. 같은 코드의 개발 모드 셸로는 확인했다(위 "운영 배포").
