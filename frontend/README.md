...
법적 판결문 분석에 특화된 논증 그래프 뷰어/에디터. 중앙 AIF 서버(backend)에 저장된 사건을 목록에서 열어 검토한다.
...
| --- | --- |
| `/` | 사건 목록 (기본 진입) |
| `/?projectId=<id>` | 그 프로젝트 검토. Desktop 의 [웹에서 열기]·게시 응답 `viewerUrl` 이 여는 주소. 새로고침·직접 접근·뒤로 가기 지원 |
| `/?view=argument` | 사이트에서 판결문을 직접 입력·분석하는 화면 (보조) |
| `/?view=pipeline` | 파이프라인 편집 (보조, `pipeline:admin` 권한이 있을 때만 탭이 보임) |

- 서버가 `AIF_AUTH_MODE=token` 이면 먼저 로그인 화면이 나온다. 주소는 그대로이므로 로그인하면 원래 보려던 프로젝트가 열린다.
  세션은 HttpOnly 쿠키이고, 화면은 CSRF 토큰만 메모리에 두어 저장 요청에 `X-CSRF-Token` 으로 붙인다(`src/auth/session.ts`).
  요청 중 401 이 오면 로그인 화면으로 돌아간다(세션 만료 안내).
- 사이트 분석 버튼은 `analysis:run`, 파이프라인 탭은 `pipeline:admin` 권한이 있는 계정에만 보인다. 서버가 `off` 면 모두 보인다.
- 다른 곳(Desktop·다른 브라우저)에서 먼저 저장해 409 가 나면 편집은 그대로 두고 배너에서 [내 편집 파일로 저장] / [서버 최신본 불러오기] 를 고른다.
- 저장 안 한 편집이 있는 채로 다른 프로젝트를 열면 확인한다.
