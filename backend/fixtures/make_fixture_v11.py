...
SCHEME_ANSWERS = [
    [
        {"ra": "R1", "scheme_key": "witness_testimony", "rationale": "신빙성이 인정된 피해자 진술이 강간 공소사실을 뒷받침한다.",
         ...
         "alternatives": [{"scheme_key": "credibility_assessment", "rationale": "신빙성 판단 자체로 볼 수도 있음"}]},
        {"ra": "R2", "scheme_key": "credibility_assessment", "rationale": "상위 명제가 쟁점의 결론(신빙성 인정)을 그대로 뒷받침한다.",
         "premise_bindings": [{"role_id": "indicators", "premises": ["N2"]}], "critical_question_responses": [], "alternatives": []},
        {"ra": "R3", "scheme_key": "credibility_assessment", "rationale": "일관성·구체성·조서와 증언의 부합이라는 판단 요소를 종합해 진술을 믿을 수 있다고 판단했다.",
         "premise_bindings": [{"role_id": "indicators", "premises": ["N3", "N4", "N5"]}],
         "critical_question_responses": [cq("CQ1", "satisfied", "현장 사진·통화 내역으로 확인됨"), cq("CQ2", "open", "")], "alternatives": []},
    ],
    [
        {"ra": "R1", "scheme_key": "abduction", "rationale": "합의라는 대안 설명이 배척되어 공소사실이 인정 사실을 가장 잘 설명한다.",
         "premise_bindings": [{"role_id": "best", "premises": ["N1"]}], "critical_question_responses": [], "alternatives": []},
        ...
        {"ra": "R3", "scheme_key": "inconsistent_commitment", "rationale": "수사 초기와 법정 진술이 번복되어 합의 주장을 신뢰하기 어렵다고 판단했다.",
         "premise_bindings": [{"role_id": "contrary", "premises": ["N3"]}], "critical_question_responses": [cq("CQ2", "open", "")], "alternatives": [{"scheme_key": "bias", "rationale": "자기방어 동기로 볼 수도 있음"}]},
    ],
    [
        {"ra": "R1", "scheme_key": "established_rule", "rationale": "폭행 요건이 충족되어 강간죄 성립 요건 중 하나가 인정된다.",
         "premise_bindings": [{"role_id": "facts", "premises": ["N1"]}], "critical_question_responses": [], "alternatives": []},
        {"ra": "R2", "scheme_key": "established_rule", "rationale": "폭행의 판례 기준에 인정 사실을 적용했다.",
         "premise_bindings": [{"role_id": "facts", "premises": ["N2"]}], "critical_question_responses": [], "alternatives": []},
