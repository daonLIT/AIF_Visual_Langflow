...
로컬 Docker(운영과 같은 구성), Windows x86_64 에서 잰 값이다. 운영 서버는 ARM64 라 **절대값은 다르다.**
비율과 병목이 어디인지가 이 표의 쓸모다. 서버 사양이나 구조가 바뀌면 같은 순서로 다시 재서 비교한다.

### 부하 시나리오 (3분씩)

| | 5명 | 50명 |
| --- | --- | --- |
| 처리량 | 1.74 req/s (313건) | 11.76 req/s (2,111건) |
| 실패 | 0% | 0% |
| `/api/health` p50 / p95 | 3 / 4ms | 3 / 93ms |
| 사건 목록 p50 | 4ms | 15ms |
| 사건 열기 p50 | 5ms | 5ms |
| 사건 저장 p50 / p95 | 49 / 150ms | 49 / 100ms |
| 게시 p50 / p95 | 95 / 110ms | 150 / 280ms |
| 로그인 p50 | 130ms | 200ms |
| 컨테이너 CPU | — | 4~31% |

50명은 한계가 아니다. 헬스 p95 가 예산(3,000ms)의 3% 고 CPU 도 코어 하나를 다 쓰지 못했다.
대기 시간이 현실적이라(검토자 2~6초, 게시 5~15초) 50명이어도 11.76 req/s 에 그친다.

### 사건 수가 늘 때 (목록)

| 사건 수 | 목록 p50 | 응답 크기 | 단건 조회 | 헬스 |
| --- | --- | --- | --- | --- |
| 191 | 19.2ms | 50KB | 3.3ms | 1.9ms |
| 500 | 47.5ms | 133KB | 3.2ms | 1.9ms |
| 1,000 | 90.0ms | 265KB | 3.2ms | 1.9ms |
| 2,000 | 184.2ms | 530KB | 3.1ms | 2.6ms |

사건당 약 0.09ms / 265바이트로 정확히 선형이었다. 단건 조회는 영향이 없다(기본키).

### 그 목록이 다른 요청을 막은 정도 (사건 2,000건, 고치기 전)
...
동시 20개를 110초 유지하자 컨테이너 헬스체크가 실제로 1회 실패했다(3초 타임아웃 초과).
3회 연속이면 `unhealthy` 로 떨어진다.

### 고친 뒤 (목록 쪽 나누기 + 목록 열 분리, 같은 2,000건)

| 측정 | 전 | 후 |
| --- | --- | --- |
| 목록 p50 | 184.2ms | 3.3ms |
| 응답 크기 | 530KB | 13.3KB |
| 동시 20건 중 헬스 p50 | 3,328ms | 43.5ms |
| 목록 처리량 | 5.8/s | 427.5/s |

헬스 여유가 예산의 111% → 1.5% 로 내려갔고, 사건이 더 늘어도 한 쪽의 응답 크기는 고정이다.

### 동시 저장 정합성

저장 76건 = 성공 29 + 충돌 47(62%), 덮어쓰기 0건. 자세한 내용은 위 "저장 충돌을 일부러 만들려면".

### 아직 재지 않은 것

- **로그인 부하** — scrypt 를 이벤트 루프에서 동기로 돌린다. 목록을 고친 지금 가장 무거운 단일 요청(200ms)이고,
  인증 없이 때릴 수 있으며 잠금은 (계정, 주소) 기준이라 계정 이름을 바꾸면 비켜 간다.
- 큰 본문 게시(상한 6MB 인데 지금 fixture 는 27KB), 30분 이상 soak, 느린 클라이언트(slowloris).
